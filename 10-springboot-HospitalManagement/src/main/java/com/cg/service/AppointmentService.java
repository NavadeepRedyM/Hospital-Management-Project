...
public class AppointmentService implements IAppointmentService {
    ...
    public void reassignDoctor(Long appointmentId, Long newDoctorId) {
        ...
        // Validation: Ensure the new doctor belongs to the same department
        if (!newDoctor.getDepartment().getId().equals(appointment.getDepartment().getId())) {
            throw new IllegalStateException("You can only reassign to a doctor within the " 
                                            + appointment.getDepartment().getDeptName() + " department.");
        }

        appointment.setDoctor(newDoctor);
        // Keep status as CONFIRMED since it's just a change of staff
        appointmentRepository.save(appointment);
    }
    @Transactional
